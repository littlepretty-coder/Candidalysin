...
data = read.delim('E:/projects/王秀丽/results/human/6h/res_human_1477_0_vs_6.txt',sep='\t')
data = na.omit(data)
data_up1 = data[(abs(data$log2FoldChange) >1.5 & data$padj <0.01),]
gene1 = rownames(data_up1)

#gene1= bitr(gene1, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")


data = read.delim('E:/projects/王秀丽/results/human/6h/res_human_2174_0_vs_6.txt',sep='\t')
data = na.omit(data)
data_up2 = data[(abs(data$log2FoldChange )>1.5 & data$padj <0.01),]
...
data = read.delim('E:/projects/王秀丽/results/human/6h/res_human_2059_0_vs_6.txt',sep='\t')
data = na.omit(data)
data_up3 = data[(abs(data$log2FoldChange )>1.5 & data$padj <0.01),]
...
data = na.omit(data)
data_up4 = data[(abs(data$log2FoldChange) >1.5 & data$padj <0.01),]
gene4 = rownames(data_up4)



gene3_entrez <- bitr(gene3, 
                     fromType = "ENSEMBL",  # 输入的基因类型
                     toType = "ENTREZID",  # 转换为 Entrez Gene ID
                     OrgDb = org.Hs.eg.db) # 使用人类注释数据库


KEGG_enrich = enrichKEGG(gene = gene3_entrez$ENTREZID, #即待富集的基因列表
                         keyType = "kegg",
...
write.table(KEGG_enrich@result,
            sprintf("E:/projects/王秀丽/results/human/kegg/kegg_6h_%s.txt",'2059'), sep = "\t")
